## MuTrans treated split: data prep
## February 2025
...
def load_anndata(file_path):
    ...
    print(f"Loaded {adata.shape} dataset from {file_path}")
    return adata

def extract_sample_name(file_path):
    base_name = os.path.basename(file_path)
    sample_name = base_name.replace("_seurat.h5ad", "")
    return sample_name

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python mutrans_2025_treated_split_prep.py <file.h5ad>")
        sys.exit(1)
